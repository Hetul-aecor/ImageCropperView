    ...
    func repositionImage() {
        ...
        if displayH >= diameter {
            if newPosition.height > verticalOffset {
                newPosition = CGSize(width: newPosition.width, height: verticalOffset - adjust + cropperConfig.inset)
                currentPosition = CGSize(width: newPosition.width, height: verticalOffset - adjust + cropperConfig.inset)
            }
            
            if newPosition.height < ( verticalOffset * -1) {
                newPosition = CGSize(width: newPosition.width, height: ( verticalOffset * -1) - adjust - cropperConfig.inset)
                currentPosition = CGSize(width: newPosition.width, height: ( verticalOffset * -1) - adjust - cropperConfig.inset)
            }
        ...
        if displayW >= diameter {
            if newPosition.width > horizontalOffset {
                newPosition = CGSize(width: horizontalOffset + cropperConfig.inset, height: newPosition.height)
                currentPosition = CGSize(width: horizontalOffset + cropperConfig.inset, height: currentPosition.height)
            }
            
            if newPosition.width < ( horizontalOffset * -1) {
                newPosition = CGSize(width: ( horizontalOffset * -1) - cropperConfig.inset, height: newPosition.height)
                currentPosition = CGSize(width: ( horizontalOffset * -1) - cropperConfig.inset, height: currentPosition.height)
            }
        ...
        ///
        if displayW < diameter - cropperConfig.inset && displayH < diameter - cropperConfig.inset {
            resetImageOriginAndScale()
        }
    }
